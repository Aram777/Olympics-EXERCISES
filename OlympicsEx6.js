...
[
   ...
   new WinterOlympics( 2020, "London",   "Great Britain" ) 
] ;
   ...
   if (isNaN(input_from_user))
   {
      ...
      switch (given_text) {
         ...
         default:
         {
            for (var index = 0; index < olympics_list.length; index++) 
            {
               //process.stdout.write(String(olympics_list[index].get_country()).toUpperCase().trim().indexOf("CHINA")+"\n");
               
               if(
                  (String(olympics_list[index].get_country()).toUpperCase().trim().indexOf(given_text)>=0) ||
                  (String(olympics_list[index].get_city()).toUpperCase().trim().indexOf(given_text)>=0) 
               )
               {
                  if ((olympics_list[index] instanceof WinterOlympics) )
                     process.stdout.write("\n    In "  +  olympics_list[index].get_year()  +
